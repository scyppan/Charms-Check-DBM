...
class BooksPage(tk.Frame):
    def __init__(self, parent, database):
        ...
        self.controller = BookController(database)
        self.records = []
    ...
    def load_record(self, record_id):
        ...
        self.status_value.set(f"Loaded {record.get('name', 'book')}")
    ...
    def new_record(self):
        ...
        self.status_value.set("Creating a new book")
    ...
    def delete_record(self):
        if self.current_record_id is None:
            ...
            self.form_dirty = False
            self.toolbar.set_record_state(dirty=False, has_record=False)
            self.status_value.set("New book cleared")
            return

        record = self.controller.get_record(self.current_record_id)
        record_name = record.get("name", "this book")
    ...
    def confirm_unsaved_changes(self):
        ...
        save_choice = messagebox.askyesnocancel(
            "Unsaved book changes",
            "Save changes to this book before continuing?",
            parent=self,
        )
