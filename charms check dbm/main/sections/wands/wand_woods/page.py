...
class WandWoodsPage(tk.Frame):
    ...
    def load_record(self, record_id):
        ...
        self.status_value.set(f"Loaded {record.get('name', 'wood')}")
    ...
    def new_record(self):
        ...
        self.status_value.set("Creating a new wand wood")
    ...
    def delete_record(self):
        if self.current_record_id is None:
            ...
            self.form_dirty = False
            self.toolbar.set_record_state(dirty=False, has_record=False)
            self.status_value.set("New wand wood cleared")
            return

        record = self.controller.get_record(self.current_record_id)
        record_name = record.get("name", "this wand wood")
    ...
    def confirm_unsaved_changes(self):
        ...
        save_choice = messagebox.askyesnocancel(
            "Unsaved wand wood changes",
            "Save changes to this wand wood before continuing?",
            parent=self,
        )
